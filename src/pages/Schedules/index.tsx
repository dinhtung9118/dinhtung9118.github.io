...
const SchedulesPage: React.FC = () => {
  ...
  const renderDetailSession = (props: any) => {
    ...
    const readOnly =
      new Date().getTime() >
      new Date(workingTime.date).getTime() + workingTime.sessions[0].to;
    if (workingTime && !workingTime.sessions) return <></>;
    const startTime =
      WorkingTime.minusFormat(workingTime.sessions[0].from || 0) || "";
    const endTime =
      WorkingTime.minusFormat(workingTime.sessions[0].to || 0) || "";
    return (
      <Box p={2}>
